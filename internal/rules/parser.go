...
var (
	ErrSyntax = fmt.Errorf("invalid rule syntax")
)
...
func (p *parser) peek() Token {
	if p.pos >= p.len { return Token{Type: EOF} }
	return p.toks[p.pos]
}
...
func (p *parser) expect(tt TokenType) (Token, error) {
	t := p.next()
	if t.Type != tt { return t, ErrSyntax }
	return t, nil
}
...
func parse(input string) (Expr, error) {
	...
	expr, err := p.parseOr()
	if err != nil { return nil, err }
	if p.peek().Type != EOF { return nil, ErrSyntax }
	return expr, nil
}
...
func (p *parser) parsePrimary() (Expr, error) {
	...
	case IDENT:
		...
		if err != nil { return nil, err }
		if _, err := p.expect(RPAREN); err != nil { return nil, err }
...
func Compile(rule string) (*Program, error) {
	if rule == "" { return &Program{expr: nil}, nil }
	e, err := parse(rule)
	if err != nil { return nil, err }
